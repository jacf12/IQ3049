...
def scenario_co2_limit(data):
    ...
    for stf in global_prop.index.levels[0].tolist():
        global_prop.loc[(stf, 'CO2 limit'), 'value'] *= 0.5
    return data
...
def scenario_co2_tax_mid(data):
    ...
    for stf in data['global_prop'].index.levels[0].tolist():
        co.loc[(stf, 'Central', 'CO2', 'Env'), 'price'] = 50
    return data
...
def scenario_north_process_caps(data):
    ...
    pro = data['process']
    for stf in data['global_prop'].index.levels[0].tolist():
        pro.loc[(stf, 'Norte', 'Hydro plant'), 'cap-up'] *= 0.5
        pro.loc[(stf, 'Norte', 'Biomass plant'), 'cap-up'] *= 0.25
    return data
